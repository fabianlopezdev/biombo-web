...
/**
 * Generate project URLs for all languages
 * @param slug - The project slug (same across all languages)
 * @returns Object with URLs for each language
 */
export function getProjectUrls(slug: string): UrlTranslations {
  return {
    ca: `/projectes/${slug}`,
    en: `/en/projects/${slug}`,
...
export function getEquivalentUrl(currentPath: string, targetLang: SupportedLanguage): string {
  ...
  if (isProjectPage) {
    ...
    const projectUrls = getProjectUrls(slug)
